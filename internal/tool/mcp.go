...
func registerRead(srv *mcpserver.MCPServer, d *dispatcher.ToolDispatcher) {
	t := mcppkg.NewTool("pf_peek",
		mcppkg.WithDescription("Read one specific resource from the memory store by URI (e.g. memory://daily/2026-04-11.md). Use after pf_scan surfaces a hit you want to read in full, or when the user names a path directly. Supports optional line-range slicing for large text resources."),
		mcppkg.WithString("uri",
			mcppkg.Description("Full resource URI including scheme. Filesystem backends use the operator-configured scheme (commonly memory://), with a path relative to the backend root: memory://2026-04-11.md, memory://projects/pagefault/design.md, memory://daily/2026-04-11.md. Copy the URI verbatim from a pf_scan hit rather than reconstructing it — custom schemes (like notes:// or archive://) are common. Trailing whitespace or an accidental leading slash breaks the lookup."),
			mcppkg.Required(),
		),
